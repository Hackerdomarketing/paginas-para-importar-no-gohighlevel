...
| **cola `width` e `max-width` numa regra inválida `width:760px%`** (Mentoria publicada, 2026-09-20) | a largura da linha é ignorada e tudo volta à largura cheia; a proporção da página inteira estraga | **largura da linha SEMPRE em porcentagem do container** (`width:N%`, sem `max-width`); Sonda K provou que é a única grafia que o importador preserva proporcional. Nunca `width:Npx; max-width:100%` (vira `width:Npx%`), nunca só `max-width` (ignorada), nunca só `width:Npx` (encolhe demais). Converter px→%: N = largura_px / container * 100, container 1120 (wide) ou 960 (mid) |
