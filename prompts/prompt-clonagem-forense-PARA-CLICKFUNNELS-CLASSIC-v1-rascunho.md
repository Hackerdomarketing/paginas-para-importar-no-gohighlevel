...
| **cola `width` e `max-width` numa regra inválida `width:760px%`** (Mentoria publicada, 2026-09-20) | a largura da linha é ignorada e tudo volta à largura cheia; a proporção da página inteira estraga | **não** escrever `width:Npx; max-width:100%` juntos; preferir `max-width:Npx` sozinho, ou `width:N%`, ou uma classe do `lander.css` (sonda curta pendente) |
